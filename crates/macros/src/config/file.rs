...
pub fn get_config() -> syn::Result<&'static Table> {
    ...
    let project_dir = std::env::var_os("CARGO_MANIFEST_DIR").ok_or_else(|| {
        Error::new(
            Span::call_site(),
            "Could not read Config.toml: Could not read `CARGO_MANIFEST_DIR` environment variable",
        )
    })?;

    let path = PathBuf::from(project_dir).join(CONFIG_TOML_PATH);
