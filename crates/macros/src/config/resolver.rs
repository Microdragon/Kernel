...
use super::input::ValueMacroInput;
...
pub fn run(query: ValueMacroInput) -> syn::Result<Expr> {
    let table = super::file::get_config()?;
    ...
    for (idx, segment) in parsed.into_iter().enumerate() {
        match current.get(segment, query.option.span())? {
...
fn parse_until<'a>(
    ...
    let idx = query
        .find(end)
        .ok_or_else(|| Error::new(span, "Quoted key is missing ending quote"))?;
    result.push(&query[..idx]);
    ...
    if !query.is_empty() {
        if !matches!(query.bytes().next(), Some(b'.')) {
            return Err(Error::new(span, "Expected '.' after quoted key"));
        }
...
impl<'a> TableOrArray<'a> {
    ...
    fn get(self, segment: &str, span: Span) -> syn::Result<Option<&'a Value>> {
        Ok(match self {
